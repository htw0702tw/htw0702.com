import {normalize,decodeEntry} from '../../server/core.mjs';
import {session,authReady,authOrigin,start,callback,hash,cookie,random} from '../../server/auth.mjs';
import {syncNotion} from '../../server/notion.mjs';
...
export async function onRequest({request:req,env}){
 ...
 if(path==='me'&&method==='GET')return json({csrf:user.csrf,connections:{auth:true,database:!!env.DB,notion:!!(env.NOTION_TOKEN&&env.NOTION_DATA_SOURCE_ID)}});
