...
enum Policy {
    ...
    static func appID(_ name: String) throws -> String {
        guard let value = apps[name] else { throw AppError("這個 App 尚未加入可操作清單。") }
        return value
    }
    static func searchURL(engine:String, query:String) throws -> URL {
        ...
        let itemName = engine == "Yahoo" ? "p" : "q"
        c.queryItems=[URLQueryItem(name:itemName,value:query)]
        guard let url=c.url else { throw AppError("無法建立搜尋網址。") }
        return url
    }
    static func sourceURL(_ string: String) throws -> URL {
        ...
        guard hosts.contains(u.host ?? "") || string == mirror || string == "https://github.com/cisagov/kev-data" else { throw AppError("來源不在允許清單；沒有開啟。") }
        return u
    }
    static func message(_ m:[String:Any]) throws {
        guard let id=m["id"] as? String, UUID(uuidString:id) != nil,
              let role=m["role"] as? String, ["user","assistant"].contains(role),
              let text=m["text"] as? String, text.utf16.count <= 24000,
              let at=m["at"] as? String, validDate(at) else { throw AppError("對話記錄格式不符。") }
    }
    ...
    static func catalog(_ object:[String:Any]) throws {
        ...
        for record in records {
            guard let id=record["cveID"] as? String, id.range(of:"^CVE-[0-9]{4}-[0-9]{4,}$",options:.regularExpression) != nil,
                  let date=record["dateAdded"] as? String, date.range(of:"^[0-9]{4}-[0-9]{2}-[0-9]{2}$",options:.regularExpression) != nil else { throw AppError("CISA 記錄缺少有效 CVE 或日期。") }
            let bytes=try JSONSerialization.data(withJSONObject:record,options:.sortedKeys)
            if let old=seen[id], old != bytes { throw AppError("CISA 同一 CVE 有衝突，未覆蓋快取。") }
            seen[id]=bytes
        }
    }
    ...
    static func serviceError(status:Int,body:Data,endpoint:String,model:String?,requestID:String?,secret:String?) -> String {
        func redact(_ value:String) -> String {
            var result=value
            if let secret=secret,!secret.isEmpty {result=result.replacingOccurrences(of:secret,with:"[已遮蔽金鑰]")}
            result=result.replacingOccurrences(of:"sk-[A-Za-z0-9_-]+",with:"[已遮蔽金鑰]",options:.regularExpression)
            return String(result.prefix(1800))
        }
        var lines=["服務回傳 HTTP \(status)。","端點：\(redact(endpoint))"]
        if let model=model {lines.append("模型：\(redact(model))")}
        if body.count<=65536,let object=try? JSONSerialization.jsonObject(with:body),let root=object as? [String:Any],let error=root["error"] as? [String:Any] {
            for (field,label) in [("code","錯誤代碼"),("type","錯誤類型"),("message","服務說明")] {
                if let value=error[field] as? String,!value.isEmpty {lines.append("\(label)：\(redact(value))")}
            }
        }
        if let id=requestID {lines.append("請求 ID：\(redact(id))")}
        if status==404 {lines.append("請核對模型 ID 與此 API 專案的存取權限；僅憑 404 尚無法確定原因。")}
        lines.append("未自動重試。")
